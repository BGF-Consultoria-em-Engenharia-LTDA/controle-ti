...
import { CardData, RequestCall, Checks, Call } from '../types/trello.ts'
import { HttpResponse } from '../utils/http.ts'
import { app } from "../index.ts";
import type { EquipmentRequest } from '../types/sheets.ts'

const factory = createFactory()
const trelloClient = new TrelloClient({
	key: Deno.env.get('TRELLO_APIKEY')!,
	token: Deno.env.get('TRELLO_APITOKEN')!
})
...
export const getTrelloCard = factory.createHandlers(async (c) => {
	...
	const TrelloCard = await trelloClient.cards.getCard({ id });
...
export const postTrelloCard = factory.createHandlers(async (c) => {
	...
	const TrelloCard = await trelloClient.cards.createCard(CardData)
	if (TrelloCard === null) return c.text('Can not create the Trello Card', 502)
	...
	if (Request['Tipo do problema/solicitação'] === 'Solicitação de equipamento para auditoria' ||
		...
		await createChecklist(TrelloCard.id, Checklist)
	}
...
export const postTrelloAction = factory.createHandlers(async (c) => {
	...
	if (Request === null) return c.text('Request is not the expected', 400)
	...
	const sheetReq = await app.request(`/sheet/${Deno.env.get('SPREADSHEET_CALLS_ID')}?start_row=${Deno.env.get('SHEET_START_ROW')}`)
	if (sheetReq.status !== 200) return c.text('Can not get the calls sheet', 502)
	const Sheet: EquipmentRequest[] = await sheetReq.json()

	const RowNumber = findRowByDescription(Sheet, CardDescription)
	if (RowNumber === null) return c.text('Call not found', 404)

	const updatedCell = await app.request(
		`/sheet/${Deno.env.get('SPREADSHEET_CALLS_ID')}/cell/AN${parseInt(Deno.env.get('SHEET_START_ROW')!) + RowNumber!}`,
		{
			method: 'put',
...
async function createChecklist(cardId: string, checks: Checks): Promise<void> {
	const TrelloChecklist = await trelloClient.checklists.createChecklist({ idCard: cardId, name: checks.name })
	if (TrelloChecklist === null) return

	console.info('Creating the Checklist')
	for (const item of checks.items) {
		await trelloClient.checklists.createChecklistCheckItems({ id: TrelloChecklist.id, name: item, checked: false })
	}
}

function findRowByDescription(sheet: EquipmentRequest[], desc: string): number | null {
